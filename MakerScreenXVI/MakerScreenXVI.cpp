...
void MakerScreenXVI::createChar(uint8_t charmap[]) {
	for (int i = 0; i < 8; i++){
		customChars[i+customCharIndex*8] = charmap[i];
	}
	customCharIndex++;
}

void MakerScreenXVI::customIcon(int charIndex){
	byte icon[8];
	for (int i = 0; i < 8; i++){
		icon[i] = customChars[charIndex*8 + i];
	}
	write(byte(1));
	createChar(1,icon);
}
